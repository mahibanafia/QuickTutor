...
{
    public partial class AddTutionN : System.Web.UI.Page
    {
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["id"] != null)
                {
                    query = @"Update Tuition set Title=@Title,Subject=@Subject,NoOfStudent=@NoOfStudent,Duration=@Duration,PerWeek=@PerWeek,
                            Salary=@Salary,TutoringMethod=@TutoringMethod,Description=@Description,
                            StuName=@StuName,Division=@Division,Mobile=@Mobile, Email=@Email,Address=@Address  where TuitionId=@id AND UserId=@UserId";
                    type = "update";
                    cmd = new SqlCommand(query, con);
